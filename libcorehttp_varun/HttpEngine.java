...
public class HttpEngine {
	...
	/*NNN 622 */
	this.wifiReqHeaders = new RequestHeaders(uri, new RawHeaders(requestHeaders));
	this.mobileReqHeaders = new RequestHeaders(uri, new RawHeaders(requestHeaders));
	/*NNN 622*/
        this.requestHeaders = new RequestHeaders(uri, new RawHeaders(requestHeaders));
    }
    /* NN 622, called everytime the request range is updated*/
    public void updateHeader(RawHeaders requestHeaders) {
		this.requestHeaders = new RequestHeaders(uri, new RawHeaders(requestHeaders));
    }
    /*NN*/
...
//	}
	...
	*/
	...
	result = HttpConnection.connect(uri, getSslSocketFactory(),
    ...
    private InputStream getTransferStream() throws IOException {
        if (!hasResponseBody()) {
            System.out.println("CSE622: From HttpEngine.java: Inside getTransferStream() - calling FixedLengthInputStream(..., ..., ..., length = 0) since httpEngine !hasResponseBody(); and return");
            return new FixedLengthInputStream(socketIn, cacheRequest, this, 0);
        }

        if (responseHeaders.isChunked()) {
            System.out.println("CSE622: From HttpEngine.java: Inside getTransferStream() - calling ChunkedInputStream(...) since httpEngine responseHeaders.isChunked(); and return");
            return new ChunkedInputStream(socketIn, cacheRequest, this);
        }

        if (responseHeaders.getContentLength() != -1) {
            System.out.println("CSE622: From HttpEngine.java: Inside getTransferStream() - calling FixedLengthInputStream(..., ..., ..., length = responseHeaders.getContentLength()) since httpEngine resp..headers.GetContLen!=-1");
    ...
    public final void readResponse() throws IOException {
		...
		if (hasResponse() && !HttpHelper.INSTANCE.useBoth) {
            System.out.println("CSE622: From HttpEngine.java: Inside readResponse() returning. httpEngine already hasResponse");
	    return;
        }
		}
        if (responseSource == null) {
        ...
        if (!responseSource.requiresConnection()) {
            System.out.println("622 - httpEngine: - I'M Here!!!");
			return;
        }
        ...
        //java.util.logging.Logger.getLogger("CSE622").info("From HttpEngine.java: Inside readResponse() calling initContentStream(getTransferStream())\n");
        System.out.println("622 - httpEngine: - Checkpoint -1");
