...
//	HttpHelper.INSTANCE.check();
	...
	/*NN 622*/
	if (httpHelper.useBoth && httpEngine2 != null) {
  	...
  	if (httpHelper.useBoth) { 
	        ...
	        try {
	            ...
	            httpEngine = newHttpEngine(method, wifiRawReqHeaders, null, null);
                    httpEngine.httpHelper = new HttpHelper("wifi");  /* needed?*/
		}catch (IOException e) {
		   wifiHttpEngineFailure = e;
		   ...
		   throw e; /* should it continue for mobile instead? */	
		}
	    }	
	    if (httpEngine2 == null) {
		...
		try {
		    ...
		    httpEngine2 = newHttpEngine(method, mobileRawReqHeaders, null, null);
	            httpEngine2.httpHelper = new HttpHelper("mobile"); /* needed?*/
		} catch (IOException e) {
		    mobileHttpEngineFailure = e;
		    ...
		    throw e; /*as above, only throw if both fail?*/
		}
	    }
	} 
	else {
	...
	if (httpHelper.useBoth) {
		...
		/*NN 622 */
		if (httpHelper.useBoth) {
                    System.out.println("CSE622: From HttpURLConnImpl - inside getResponse() sending request for httpengine");
                    HttpHelper.INSTANCE.connectionFor("wifi");
	            wifiRawReqHeaders.set("Range", "bytes=0-10");   
		    ...
		    httpEngine.sendRequest();
	   	    ...
	   	    HttpHelper.INSTANCE.connectionFor("mobile");
		    mobileRawReqHeaders.set("Range", "bytes=100-200");
