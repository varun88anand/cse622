...
final class HttpConnection {
    ...
    private HttpConnection(Address config, int connectTimeout) throws IOException {
        ...
        System.out.println("CSE622 From HttpConnection.java: Inside constr.");
        /*
        ...
        for (int i = 0; i < addresses.length; i++) {
            System.out.println("CSE622 From HttpConnection.java: Creating new Socket and trying all addresses.");
	  ...
	  /*NNN 622 new NNN*/
		...
		if(HttpHelper.INSTANCE.useBoth) {
	        ...
	        InetAddress localAddress;
                InetSocketAddress localSockAddress;
		try {
		    fd = Libcore.os.socket(AF_INET, SOCK_DGRAM, 0);
		    if (HttpHelper.INSTANCE.WIFI) {
                localAddress = Libcore.os.ioctlInetAddress(fd, SIOCGIFADDR, "wlan0");
		        System.out.println("CSE622: Using Wifi address:  " + localAddress.getHostAddress() );
		    	HttpHelper.INSTANCE.wifiSocket = socketCandidate;
		        localSockAddress = new InetSocketAddress(localAddress, 0);
		        socketCandidate.bind((SocketAddress)localSockAddress);
		    }
		    else if (HttpHelper.INSTANCE.MOBILE) {
				localAddress = Libcore.os.ioctlInetAddress(fd, SIOCGIFADDR, "cdma_rmnet4");
                System.out.println("CSE622: Using Mobile address:  " + localAddress.getHostAddress() );
		   		HttpHelper.INSTANCE.mobileSocket = socketCandidate;
				localSockAddress = new InetSocketAddress(localAddress, 0);
		        socketCandidate.bind((SocketAddress)localSockAddress);
		    }	
